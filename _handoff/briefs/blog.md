...
- **title:** `Aesthetic & Skincare Blog - Marin Medical Aesthetics`
- **meta description:** `Skincare and treatment articles from Marin Medical Aesthetics in San Anselmo, CA, written to help you arrive at a consultation with better questions.`
- **primary keyword:** `Aesthetic & Skincare Blog`
...
links_to: [areas-of-concern, consultation]
links_from: [index]
```

> **Filter-field set (registry check 2c).** `filter_fields: category, tags, date` — `category` and `tags` are always required (they bind to `data-cat` / `data-tags`). The sort offers date-ordering, so per registry check (2c) the item template MUST also carry `date` (-> `data-date`). Declared above.

## Outcome row (required)

FEATURE: the practice's written articles, listed honestly and filterable on fields that carry real values  →  DO: browse what genuinely exists rather than a styled grid  →  MEANS: the filter returns real items, and an empty state says so instead of pretending.
...
> **PAGE CAUTION.** ⚠The estate `/blog/` page has H1 count 0 — one of exactly four H1-less pages (must_fix #5). The build MUST author a real H1.
