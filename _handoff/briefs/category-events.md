# category-events — P6 typed content brief
...
- **meta description:** `Explore upcoming events, wellness workshops, and educational experiences at Marin Medical Aesthetics in Marin County. Join us for community events.`
- **primary keyword:** `Events`
...
links_to: [consultation, contact]
links_from: [index]
```

> **Filter-field set (registry check 2c).** `filter_fields: category, tags, date` — `category` and `tags` are always required (they bind to `data-cat` / `data-tags`). Date-ordering offered, so `date` is declared per check (2c). An events list is the one index where a stale item is actively misleading.

## Outcome row (required)

FEATURE: practice events, listed honestly and filterable on fields that carry real values  →  DO: browse what genuinely exists rather than a styled grid  →  MEANS: the filter returns real items, and an empty state says so instead of pretending.
...
> **PAGE CAUTION.** ⚠Never ship an empty filter. Registry floor: "never ship a filter with 0 items (FLAG empty)". If no event is current, the page states that plainly rather than rendering an empty grid. No captured competitor publishes an events archive, so there is no precedent to copy — this exists only because the client already publishes it.
...
- **item-template field set declared:** yes — `filter_fields: category, tags, date`.
