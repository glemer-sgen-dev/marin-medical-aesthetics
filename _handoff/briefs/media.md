...
- **title:** `In the Media - Marin Medical Aesthetics`
- **meta description:** `Press coverage and media features about Marin Medical Aesthetics and Dr. Chris Bacchi, MD, in San Anselmo, CA.`
- **primary keyword:** `In the Media`
...
links_to: [about, consultation]
links_from: [index]
```

> **Filter-field set (registry check 2c).** `filter_fields: category, tags, date` — `category` and `tags` are always required (they bind to `data-cat` / `data-tags`). Date-ordering offered, so `date` is declared on the item template per check (2c).

## Outcome row (required)

FEATURE: press and media appearances, listed honestly and filterable on fields that carry real values  →  DO: browse what genuinely exists rather than a styled grid  →  MEANS: the filter returns real items, and an empty state says so instead of pretending.
...
> **PAGE CAUTION.** ⚠PROOF-CLASS PAGE. Every item is an external claim about the practice. `D-BRAND-RAILS-013` real-only applies absolutely: no invented outlet, date, headline or logo, and no item without a verifiable source. Anything testimonial or outcome-bearing inherits F7.
